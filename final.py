        ...
        while True:
            ...
            if not self.move_plan.isRunning() and len(self.wp_list)>1\
                ...
                if not self.nwp and self.app.queue[0]['f'] <=5 and\
                    ...
                    while self.app.queue[0]['f'] <= 5 and self.app.queue[0]['b'] <= 5:
                        yield self.forDuration(1)
                        if self.app.queue[0]['f'] <= 5 and self.app.queue[0]['b'] <= 5:
                    ...
                    if len(self.app.latest_w) == len(self.wp_list):
                        ...
                        if biggest2 < biggest:
                            self.app.direction = -1 * self.app.direction
                            self.move_plan.direction = self.app.direction
                        while len(self.app.latest_w) == len(self.wp_list):
                            self.move_plan.start(duration=1)
                            yield self.forDuration(8)
            ...
            yield self.forDuration(0.6) # change to 0.3 if necessary
  ...
  def behavior( self ):
    while True:
     ...
     # print "fb"
      ...
      ab = mean(self.app.b_queue[:1])
      progress("ab = %s" % ab)
      self.app.ab_queue.insert(0,ab)
      af = mean(self.app.f_queue[:1])
      progress("af = %s" % af)
